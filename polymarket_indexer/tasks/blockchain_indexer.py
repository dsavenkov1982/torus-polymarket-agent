...
# Smart contract ABIs (simplified for key events)
...
class PolygonBlockchainIndexer:
    def __init__(self, settings):
        """
        Initialize blockchain indexer with Web3 connection and SQL indexer.
        """
        ...
        logger.info(f"Blockchain indexer initialized. Connected to Polygon: {self.w3.is_connected()}")

    async def initialize(self):
        """Initialize SQL indexer connection."""
    ...
    async def index_blockchain_data(self) -> None:
        """
        Main indexing function - processes new blocks and events.
        """
        try:
            ...
            # Update market metrics for recently active markets
            await self._update_recent_market_metrics()

            logger.info("Blockchain indexing cycle completed successfully")
    ...
    async def _process_conditional_tokens_events(self, current_block: int) -> None:
        """
        Process ConditionalTokens contract events.
        """
        ...
        logger.info(f"Processing ConditionalTokens events: blocks {start_block}-{end_block}")

        events_processed = 0

        try:
            # Process ConditionPreparation events (new markets)
            ...
            # Process ConditionResolution events (market settlements)
            ...
            # Process TransferSingle events (balance updates)
            ...
            logger.info(f"Processed {events_processed} ConditionalTokens events in blocks {start_block}-{end_block}")
    ...
    async def _process_ctf_exchange_events(self, current_block: int) -> None:
        """
        Process CTF Exchange contract events.
        """
        ...
        logger.info(f"Processing CTF Exchange events: blocks {start_block}-{end_block}")

        events_processed = 0
        trades_batch = []

        try:
            # Process OrderFilled events (trades)
            ...
            logger.info(f"Processed {events_processed} CTF Exchange events in blocks {start_block}-{end_block}")
    ...
    async def _handle_condition_preparation(self, event) -> None:
        """
        Handle ConditionPreparation event (new market creation).
        """
        try:
            args = event['args']
            ...
            condition_data = {
                ...
                # Metadata will be enriched later via external APIs
                'question': None,
                'description': None,
                'end_date': None,
                'category': None,
            ...
            logger.info(f"New market created: {condition_data['condition_id'][:10]}...")

        except Exception as e:
            logger.error(f"Error handling ConditionPreparation event: {e}")
            raise

    async def _handle_condition_resolution(self, event) -> None:
        """
        Handle ConditionResolution event (market settlement).
        """
        try:
            args = event['args']
            block_info = self.w3.eth.get_block(event['blockNumber'])
            ...
            logger.error(f"Error handling ConditionResolution event: {e}")
            raise

    async def _handle_token_transfer(self, event) -> None:
        """
        Handle TransferSingle event (position token transfer).
        """
        try:
            args = event['args']
            block_info = self.w3.eth.get_block(event['blockNumber'])
            ...
            logger.warning(f"Error handling TransferSingle event: {e}")

    async def _handle_trade_event(self, event) -> Dict[str, Any]:
        """
        Handle OrderFilled event (trade execution).
        """
        try:
            args = event['args']
            block_info = self.w3.eth.get_block(event['blockNumber'])
            ...
            trade_data = {
                ...
                'trader': args['taker'],  # The person taking the trade
                ...
                'price': price,
                'is_buy': args['side'] == 0,  # 0 = buy, 1 = sell
                'order_id': None  # Would need additional data for order ID
            }
    ...
    async def _update_recent_market_metrics(self) -> None:
        """
        Update metrics for markets that had recent trading activity.
        """
        try:
            # Get conditions with recent trades (last hour)
            ...
            logger.info(f"Updated metrics for {len(recent_conditions)} recently active markets")

        except Exception as e:
            logger.warning(f"Error updating recent market metrics: {e}")


@shared_task(name="blockchain_indexer.run_polymarket_indexer")
def run_polymarket_indexer():
    """
    Celery task to run the Polymarket blockchain indexer.
    This task runs periodically to index new blockchain data.
    """
    ...
    try:
        ...
        logger.info("Polymarket indexing task completed successfully")
    except Exception as e:
        logger.error(f"Error in Polymarket indexing task: {e}")
        raise
    finally:
        loop.close()
...
async def index_polymarket_data():
    """
    Main async function to run the blockchain indexing process.
    """
...
# Metadata enrichment task (optional)
@shared_task(name="blockchain_indexer.enrich_market_metadata")
def enrich_market_metadata():
    """
    Celery task to enrich market metadata from external sources.
    This runs less frequently than the main indexer.
    """
    logger.info("Starting market metadata enrichment task")
    ...
    try:
        ...
        logger.info("Metadata enrichment task completed successfully")
    except Exception as e:
        logger.error(f"Error in metadata enrichment task: {e}")
        raise
    finally:
        loop.close()
...
async def enrich_metadata():
    """
    Enrich market metadata from external APIs and IPFS.
    """
    indexer = PolygonBlockchainIndexer(settings)
    ...
    try:
        ...
        for market in markets_to_enrich:
            try:
                # Here you would call external APIs to get metadata
                # For now, we'll use placeholder logic
                metadata = await fetch_market_metadata(market['question_id'])

                if metadata:
                    await indexer.sql_indexer.enrich_market_metadata(
                        market['condition_id'],
                        metadata
                    )

            except Exception as e:
                logger.warning(f"Failed to enrich metadata for {market['condition_id']}: {e}")
                continue
...
async def fetch_market_metadata(question_id: str) -> Dict[str, Any]:
    """
    Fetch market metadata from external sources.

    Args:
        question_id (str): The question ID to look up.

    Returns:
        Dict: Market metadata or None if not found.
    """
    try:
        # Placeholder for actual API calls
        # In reality, you would:
        # 1. Query UMA protocol for question details
        # 2. Query Polymarket API for market info
        ...
        logger.warning(f"Error fetching metadata for question {question_id}: {e}")
        return None
...
def database_maintenance():
    """
    Celery task for database maintenance and cleanup.
    """
...
async def run_maintenance():
    ...
    try:
        ...
        await indexer.sql_indexer.cleanup_old_data(days_to_keep=90)

        # Update metrics for all active markets (daily refresh)
...
async def manual_index_test():
    """
    Manual function to test the indexer (for development).
    """
    logger.info("Running manual indexer test")
    ...
    try:
        ...
        # Get some stats
        stats = await indexer.sql_indexer.get_indexer_stats()
        logger.info(f"Indexer stats: {stats}")
